...
$sourceDir = $RootPath
$targetDir = Join-Path $parentDir ".cursor\rules"
...
# Run sync report cleanup
Write-Host "Running sync report cleanup..." -ForegroundColor Cyan
...
return @{
    ...
    OrphanCount = $orphanedMdcFiles.Count
    RemovedOrphanCount = $removedOrphanFiles.Count
