...
The simplified three-command system follows a standardized space-delimited format with five standard components and an optional prompt component:
...
prompt: Focus specifically on message-command improvements`
...
prompt: Additional guidance or instructions`
   ...
   - Example with prompt: `dev-mode rules-workflow system-wide-optimization @parameters/rules/dev-mode/system-wide-optimization.mdc none
prompt: Ensure backwards compatibility with existing commands`
...
5. **Five-Component Structure**: All message-commands must have exactly five standard components
   - mode
   - workflow-type
   - pathway-name
...
6. **Optional Prompt Component**: The prompt component can be added after the five standard components
   - Must begin on a new line
   - Must start with "prompt:" prefix followed by a space
   - Can contain multiple lines of text
   - Example: 
     ```
     plan-mode rules-workflow system-wide @parameters/rules/plan-mode/system-wide.mdc none
     prompt: Focus on improving the message-command system
     ```
...
4. **Prompt Component**:
   ...
   - Starts on a new line after the five standard components
   - Begins with "prompt:" prefix
   - Used to provide context, guidance, or clarification
   - Example:
     ```
     dev-mode rules-workflow performance @parameters/rules/dev-mode/performance.mdc none
     prompt: Focus on optimizing the file reading mechanisms
     ```
...
| Format | mode workflow-type pathway-name @parameter.mdc optional-params [optional prompt] | emoji 1000xdev [workflow-type] |
| Example | plan-mode rules-workflow system-wide @parameters/rules/plan-mode/system-wide.mdc none | 📋 1000xdev [rules-workflow] |
| Example with prompt | plan-mode rules-workflow system-wide @parameters/rules/plan-mode/system-wide.mdc none<br>prompt: Additional guidance | 📋 1000xdev [rules-workflow] |
...
The prompt component allows providing additional guidance while maintaining the current mode:

1. **When to Use**:
   - When additional guidance is needed without changing modes
   - To provide context for complex implementations
   - To clarify specific aspects of a task
   - To adjust focus within the current operation

2. **Format Requirements**:
   - Must begin with "prompt:" on a new line after standard components
   - No special formatting required for content
   - Can span multiple lines if needed
   - No limit on length, but conciseness is preferred

3. **Processing Behavior**:
   - Prompt is processed as additional context, not as a new command
   - Does not change current mode or workflow
   - Supplements but does not override project-rule-parameter guidance
   - Relevant only for the current message-command
