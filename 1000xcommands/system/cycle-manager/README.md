# Cycle Manager System

The Cycle Manager (`cycle-manager`) is a standardized system for creating, enhancing, and optimizing cycles across all domains. It implements a unified cycle model that enables both autonomous and user-directed operation.

## Purpose

The `cycle-manager` serves as the central system for:

1. **Creating new cycles** that follow standardized patterns
2. **Enhancing existing cycles** while maintaining standards compliance
3. **Optimizing cycles** to reduce clutter and improve efficiency
4. **Standardizing cycle operations** across all domains

## Cycle Structure

The `cycle-manager` follows the standard 7-step sequential command pattern:

1. **Cycle Initiation**: Initialize cycle and prepare environment
2. **Requirement Analysis**: Analyze requirements and determine execution mode
3. **Planning**: Create implementation plan
4. **Implementation**: Execute planned changes
5. **Verification**: Verify changes meet requirements
6. **Refinement**: Address issues and optimize implementation
7. **Completion**: Finalize and document results

## Unified Model

All cycles managed by the `cycle-manager` implement a unified model that supports both autonomous and user-directed operation:

- **User-Directed Mode**: Processes user input from the USER REQUEST SECTION
- **Autonomous Mode**: Executes based on documented potential enhancements

The mode is determined automatically in step 2 based on the presence or absence of user input.

## Usage

### Creating a New Cycle

```
run command:system/cycle-manager/1
```

Fill in the USER REQUEST SECTION in notes.md with details about the new cycle to create.

### Enhancing an Existing Cycle

```
run command:system/cycle-manager/1
```

Fill in the USER REQUEST SECTION in notes.md with details about the cycle to enhance.

### Optimizing Cycles

```
run command:system/cycle-manager/1
```

Fill in the USER REQUEST SECTION in notes.md with details about the optimization needed.

### Running Autonomously

```
run command:system/cycle-manager/1
```

Leave the USER REQUEST SECTION empty, and the cycle will run autonomously based on documented potential enhancements.

## Knowledge and Processes

The `cycle-manager` centralizes cycle-related knowledge and processes:

- **Knowledge**: `/1000xbrain/system/cycle-manager/knowledge/`
- **Processes**: `/1000xbrain/system/cycle-manager/processes/`
- **Guidelines**: `/1000xbrain/system/guidelines/`

## Implementation Details

- **Standardized File Structure**: All cycles follow the same directory structure
- **Unified Naming Conventions**: Consistent naming across all domains
- **Centralized Knowledge**: Shared knowledge and processes
- **Enhancement Documentation**: All cycles document potential enhancements

## Benefits

1. **Consistency**: Ensures all cycles follow the same structure and patterns
2. **Flexibility**: Supports both autonomous and user-directed operation
3. **Maintainability**: Centralizes cycle knowledge for easier updates
4. **Efficiency**: Reduces duplication and improves cycle quality
5. **Standardization**: Provides a unified approach across all domains 