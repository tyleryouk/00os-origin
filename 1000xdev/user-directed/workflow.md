...
> **Workflow Boundary Notice:**
> - The `user-directed/` folder is read-only and is the authoritative source of requirements and goals for each cycle.
> - Step 1 of every cycle is to read all files in `user-directed/` and synthesize them into `active-request.md` (never edit `user-directed/` directly).
> - Only core workflow files (`README.md`, `user-rules-1000xdev.md`, `documentation/`, etc.) are to be updated by 1000xdev.
> - This separation ensures a clear, auditable record of requirements and implementation, and prevents accidental modification of the user-authored roadmap.
