...
description: Reads the current task defined in 00reaper/current-task.md and executes the necessary changes to 00OS process files and tracking documents.
version: 3.0.0
category: 00reaper
author: 00reaper
...
1. Assumes the context of the `00reaper/` directory has been loaded (e.g., by a preceding `> reaper-read-files` command, potentially storing results in process state).
2. Reads the specific objective from `00reaper/current-task.md`.
...
async function execute(args, flags) {
  ...
  const actionsTaken = [];
  const errorsEncountered = [];
  const taskFilePath = "00reaper/current-task.md";
  ...
  try {
    ...
    try {
      ...
      return formatError(
        ...
        error.suggestions || [
          "Ensure the current-task.md file exists in 00reaper/",
          "Run > reaper-os-commands-workflow to initialize or update the workflow files"
        ]
      );
    }
...
### Missing Task File
```
❌ Error [FILE_READ_ERROR]: Critical error: Failed to read 00reaper/current-task.md: File not found

Suggestions:
- Ensure the file exists at 00reaper/current-task.md
- Run > reaper-os-commands-workflow to initialize or update the workflow files
...
Actions Taken (5):
- Read task from 00reaper/current-task.md.
...
- Failed action: edit_file on 00reaper/command-registry.md. Reason: Could not find file for editing (Suggestions: Verify that the target file path is correct; Check if the directory exists and create it if needed)
