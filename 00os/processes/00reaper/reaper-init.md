 ...
 * @param {Object} tracker - Content tracker
 */
async function loadCoreWorkflowFiles(params, tracker) {
  try {
    // Define core workflow files
    const coreFiles = [
      {
        path: '00reaper/00OS-commands/README.md',
        property: 'readme',
        explanation: 'Loading core workflow README'
      },
      {
        path: '00reaper/00OS-commands/active-request.md',
        property: 'activeRequest',
        explanation: 'Loading active request definition'
      },
      {
        path: '00reaper/00OS-commands/implementation-plan.md',
        property: 'implementationPlan',
        explanation: 'Loading implementation plan'
      },
      {
        path: '00reaper/00OS-commands/cycle-status.md',
        property: 'cycleStatus',
        explanation: 'Loading cycle status information'
      }
    ];
    
    // Load each core workflow file
    for (const file of coreFiles) {
      try {
        ...
        if (fileContent && fileContent.content) {
          ...
          tracker.coreWorkflow[file.property] = {
            path: file.path,
          ...
          if (file.property === 'activeRequest') {
            analyzeActiveRequest(tracker, fileContent.content);
          } else if (file.property === 'cycleStatus') {
            analyzeCycleStatus(tracker, fileContent.content);
          }
        ...
        console.error(`Error loading core workflow file ${file.path}: ${error.message}`);
        tracker.stats.loadFailures++;
      }
    }
  } catch (error) {
    console.error(`Error in core workflow file loading: ${error.message}`);
    // Continue with other parts of initialization
  }
}

/**
 * Analyze active request content
 * @param {Object} tracker - Content tracker
 * @param {string} content - File content
 */
function analyzeActiveRequest(tracker, content) {
  try {
    // Extract request ID
    const requestIdMatch = content.match(/^# (REQ-\d+):/m);
    if (requestIdMatch) {
      const requestId = requestIdMatch[1];
      
      // Store in tracker
      if (!tracker.analysisResults) tracker.analysisResults = {};
      tracker.analysisResults.activeRequestId = requestId;
      
      // Extract parts and requirements
      const parts = [];
      ...
      for (const line of lines) {
        ...
        } else if (currentPart && line.match(/^\d+\.\s+\*\*.*?\*\*/)) {
          const requirement = line.replace(/^\d+\.\s+\*\*/, '').replace(/\*\*/, '');
          currentPart.requirements.push(requirement);
        }
      }
      
      if (parts.length > 0) {
        tracker.analysisResults.requestParts = parts;
      }
    }
  } catch (error) {
    console.error(`Error analyzing active request: ${error.message}`);
  }
}

/**
 * Analyze cycle status content
 * @param {Object} tracker - Content tracker
 * @param {string} content - File content
 */
function analyzeCycleStatus(tracker, content) {
  try {
    // Extract current stage
    const stageMatch = content.match(/\*\*CURRENT CYCLE STAGE\*\*:\s*(.*)/);
    if (stageMatch) {
      const currentStage = stageMatch[1];
      
      // Store in tracker
      if (!tracker.analysisResults) tracker.analysisResults = {};
      tracker.analysisResults.currentStage = currentStage;
      
      // Extract current focus
      const focusMatch = content.match(/\*\*Current Focus\*\*:\s*(.*)/);
      if (focusMatch) {
        tracker.analysisResults.currentFocus = focusMatch[1];
      }
      
      // Extract next action
      const nextActionMatch = content.match(/\*\*Next Action\*\*:\s*(.*)/);
      if (nextActionMatch) {
        tracker.analysisResults.nextAction = nextActionMatch[1];
      }
    }
  } catch (error) {
    console.error(`Error analyzing cycle status: ${error.message}`);
  }
}
...
function generateInitializationReport(params, tracker) {
  try {
    ...
    // Count loaded core workflow files
    const loadedCoreFiles = Object.values(tracker.coreWorkflow).filter(item => item !== null).length;
    
    // Generate basic report
    let report = `✅ 00reaper Initialization Complete\n\n`;
    
    // Add current workflow context if available
    if (tracker.analysisResults) {
      report += `Current Workflow Context\n`;
      report += `======================\n`;
      
      if (tracker.analysisResults.activeRequestId) {
        report += `Active Request: ${tracker.analysisResults.activeRequestId}\n`;
      }
      
      if (tracker.analysisResults.currentStage) {
        report += `Current Stage: ${tracker.analysisResults.currentStage}\n`;
      }
      
      if (tracker.analysisResults.currentFocus) {
        report += `Current Focus: ${tracker.analysisResults.currentFocus}\n`;
      }
      
      if (tracker.analysisResults.nextAction) {
        report += `Next Action: ${tracker.analysisResults.nextAction}\n`;
      }
    ...
    report += `====================\n`;
    report += `Core Workflow Files: ${loadedCoreFiles}\n`;
    ...
    if (params.verbose) {
      ...
      // Core workflow files breakdown
      report += `Core Workflow Files (${loadedCoreFiles})\n`;
      report += `-------------------\n`;
      Object.entries(tracker.coreWorkflow).forEach(([key, value]) => {
        if (value) {
          report += `- ${value.name}\n`;
        }
      });
      ...
      // If request parts were analyzed, show details
      if (tracker.analysisResults && tracker.analysisResults.requestParts) {
        report += `Active Request Details\n`;
        report += `---------------------\n`;
        
        tracker.analysisResults.requestParts.forEach((part, index) => {
          report += `${part.title}\n`;
          
          part.requirements.forEach((req, reqIndex) => {
            report += `  ${reqIndex + 1}. ${req}\n`;
          });
