 ...
 */
function formatSyncResults(output, error, exitCode, isVerbose, isDryRun) {
  if (exitCode !== 0) {
    return `❌ Synchronization failed with exit code ${exitCode}\n\nError:\n${error}`;
  }

  // Parse the output to extract key statistics
  const stats = extractSyncStats(output);
...
Files processed: ${stats.processed || 'unknown'}
Files updated: ${stats.updated || 0}
Files created: ${stats.created || 0}
Files skipped: ${stats.skipped || 0}
Errors: ${stats.errors || 0}
`;

  // If verbose, include the entire output
  if (isVerbose) {
    result += `\nDETAILED OUTPUT\n${'-'.repeat(20)}\n${output}`;
  } else {
    result += `\nUse 'reaper-sync --verbose' for detailed output.`;
  }
...
/**
 * Extract synchronization statistics from the output
 */
function extractSyncStats(output) {
  const stats = {
    processed: 0,
    updated: 0,
    created: 0,
  ...
  const summaryMatch = output.match(/Total files processed: (\d+)\s+Files updated: (\d+)\s+Files created: (\d+)\s+Files skipped: (\d+)\s+Errors: (\d+)/);
  
  if (summaryMatch) {
    stats.processed = parseInt(summaryMatch[1]);
    stats.updated = parseInt(summaryMatch[2]);
    stats.created = parseInt(summaryMatch[3]);
    stats.skipped = parseInt(summaryMatch[4]);
    stats.errors = parseInt(summaryMatch[5]);
  }
...
/**
 * Logs a synchronization event to the sync history
 */
function logSyncEvent(message) {
  // In a real implementation, this would append to a sync history file
